...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tombol'])) {
    ...
    $queryOld = mysqli_query($connect, "SELECT * FROM about_us WHERE id='$id'") or die(mysqli_error($connect));
    ...
    if (!empty($_FILES['banner']['tmp_name'])) {
        ...
        move_uploaded_file($_FILES['banner']['tmp_name'], "../../../storages/about_us/" . $bannerName);
    }

    // Upload logo baru jika ada
    if (!empty($_FILES['logo']['tmp_name'])) {
        $logoName = uniqid() . "_logo.png";
        move_uploaded_file($_FILES['logo']['tmp_name'], "../../../storages/about_us/" . $logoName);
    }

    // Query Update data
    $qUpdate = "UPDATE about_us SET 
                    judul='$judul',
                    deskripsi='$deskripsi',
		            kategori='$kategori',
		            text='$text',
